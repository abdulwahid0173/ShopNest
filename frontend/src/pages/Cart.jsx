...
import { removeFromCart, setCartQty } from '../redux/cartSlice';
...
const Cart = () => {
  ...
  const handleRemove = (id) => {
    const item = cartItems.find((cartItem) => (cartItem.productId || cartItem.id) === id);
    ...
    if ((item.qty || 1) > 1) {
      dispatch(setCartQty({ itemId: id, qty: item.qty - 1 }));
    } else {
  ...
  const handleUpdateQty = (item, qty) => {
    if (qty > 0) {
      dispatch(setCartQty({ itemId: item.productId || item.id, qty }));
    }
  };
